// import 'package:geocoding/geocoding.dart';
// import 'package:http/http.dart' as http;
// import 'package:places_autocomplete/src/models/place.dart';
// import 'dart:convert' as convert;

// import 'package:places_autocomplete/src/models/place_search.dart';

// class PlacesService {
//   final key = 'AIzaSyDM8U1e_9FPJqaCu4Vv0YrMxj6vqEyWyiA';

//   Future<List<PlaceSearch>> getAutocomplete(String search) async {
//     String type = 'country:Lk';
//     var url =
//         'https://maps.googleapis.com/maps/api/place/autocomplete/json?input=$search&types=(cities)&key=$key&components=$type';
//     var response = await http.get(Uri.parse(url));
//     var json = convert.jsonDecode(response.body);
//     var jsonResults = json['predictions'] as List;
//     return jsonResults.map((place) => PlaceSearch.fromJson(place)).toList();
//   }

//   Future<Place> getPlace(String placeId) async {
//     var url =
//         'https://maps.googleapis.com/maps/api/place/details/json?place_id=$placeId&key=$key';
//     var response = await http.get(Uri.parse(url));
//     var json = convert.jsonDecode(response.body);
//     var jsonResult = json['result'] as Map<String, dynamic>;
//     return Place.fromJson(jsonResult);
//   }

//   Future<List<Place>> getPlaces(
//       double lat, double lng, String placeType) async {
//     print(getPlaces);
//     var url =
//         'https://maps.googleapis.com/maps/api/place/textsearch/json?location=$lat,$lng&type=$placeType&rankby=distance&key=$key';
//     var response = await http.get(Uri.parse(url));
//     var json = convert.jsonDecode(response.body);
//     print("safasf");
//     print("dasdfasf");
//     print(response.body);

//     var jsonResults = json['results'] as List;
//     return jsonResults.map((place) => Place.fromJson(place)).toList();
//   }
// }
